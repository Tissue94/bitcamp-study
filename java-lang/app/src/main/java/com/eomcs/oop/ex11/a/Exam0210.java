...
public class Exam0210 {
  ...
  public static void main(String[] args) {
    ...
    // 4) anonymous class 
    // => 클래스의 이름이 없다. 
    //    이름이 없으니, 따라서 new 명령으로 따로 인스턴스를 생성할 수 없다.
    // => 문법 = 클래스를 정의하는 문법 + 인스턴스를 만드는 문법이 결합되어 있다.
    //    즉 클래스를 정의하는 동시에 인스턴스를 생성해야 한다.
    // => 클래스 이름이 없기 때문에 익명클래스는 생성자를 정의할 수 없다.
    // => 만약 인스턴스의 값을 초기화시키기 위해 복잡한 코드를 작성해야 한다면,
    //    "인스턴스 블록"을 사용하여 인스턴스 초기화 코드를 작성하라!
    // => 단 한 개의 인스턴스만 생성해서 사용할 경우 익명 클래스를 적용한다.
    // => 문법
    //      new 수퍼클래스() {수퍼클래스를 상속받은 익명 클래스 정의}
    //      new 인터페이스() {인터페이스를 구현한 익명클래스 정의}
    // => 주의!
